...
class BaselineChannelAttention(nn.Module):
    def __init__(self, in_planes, ratio=16):
        super(BaselineChannelAttention, self).__init__()
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        self.max_pool = nn.AdaptiveMaxPool2d(1)
...
class BaselineCBAM(nn.Module):
    def __init__(self, in_planes, ratio=16, kernel_size=7):
        super(BaselineCBAM, self).__init__()
        self.ca = BaselineChannelAttention(in_planes, ratio)
        self.sa = BaselineSpatialAttention(kernel_size)
...
class BaselineCBAMResNet50(nn.Module):
    def __init__(self, num_classes=5):
        super(BaselineCBAMResNet50, self).__init__()
        try:
            resnet = models.resnet50(weights=None)
        except TypeError:
            resnet = models.resnet50(pretrained=False)
        ...
        self.maxpool = resnet.maxpool
        self.layer1 = resnet.layer1
        self.cbam1 = BaselineCBAM(256)
        self.layer2 = resnet.layer2
        self.cbam2 = BaselineCBAM(512)
        self.layer3 = resnet.layer3
        self.cbam3 = BaselineCBAM(1024)
        self.layer4 = resnet.layer4
        self.cbam4 = BaselineCBAM(2048)
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
...
def load_trained_model(model_path):
    ...
    try:
        ...
        if isinstance(model_data, dict) or not hasattr(model_data, 'eval'):
            ...
            has_cbam1 = 'cbam1.ca.fc.0.weight' in keys
            has_cbam3_mlp = 'cbam3.ca.mlp.0.weight' in keys
            has_fc1_fc2 = 'fc1.weight' in keys and 'fc2.weight' in keys
            
            if has_cbam3_mlp and has_fc1_fc2:
                st.success("Detected custom CBAM-ResNet50 (2 CBAM layers + Double FC) architecture!")
                model = CBAMResNet50(num_classes=len(CLASS_LABELS))
            elif has_cbam1:
                st.success("Detected custom Baseline CBAM-ResNet50 (4 CBAM layers) architecture!")
                model = BaselineCBAMResNet50(num_classes=len(CLASS_LABELS))
            else:
